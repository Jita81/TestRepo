...
**Platform note:** Agent-facing retrieval should combine **semantic indexes** with **indexed regex / text search** (trigram-style and successors); see [agent-context-retrieval.md](agent-context-retrieval.md) and **§07** in [context-platform-process-architecture.md](context-platform-process-architecture.md). **Repo (Phase 11):** [codebase-index-phase11.md](codebase-index-phase11.md) — CLI mirror + **`/codebase-search`** + verify (substring `LIKE` MVP).
